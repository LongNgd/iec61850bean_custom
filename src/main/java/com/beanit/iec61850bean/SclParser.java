...
public class SclParser {
  ...
  private LogicalNode createNewLogicalNode(Node lnXmlNode, String parentRef)
    ...
    // look for GSEControl (GOOSE) definitions
    for (int i = 0; i < lnXmlNode.getChildNodes().getLength(); i++) {
      Node childNode = lnXmlNode.getChildNodes().item(i);
  ...
  private List<Rcb> createReportControlBlocks(Node xmlNode, String parentRef)
    ...
    for (int z = 1; z <= maxInstances; z++) {
      ...
      BdaInt32U confRef = new BdaInt32U(
  ...
  private List<FcDataObject> createFcDataObjects(
    ...
    for (Sdo sdo : doType.sdos) {
      ...
      // parsing Arrays of SubDataObjects is ignored for now because no SCL file was
      // found to test
      // against. The
      // only DO that contains an Array of SDOs is Harmonic Value (HMV). The Kalkitech
      // SCL Manager
      // handles the
  ...
  private FcModelNode createDataAttribute(
    ...
    if (bType.equals("BOOLEAN")) {
      ...
      BdaVisibleString bda = new BdaVisibleString(
      ...
      BdaUnicodeString bda = new BdaUnicodeString(
          new ObjectReference(ref),
          fc,
          sAddr,
      ...
      BdaOctetString bda = new BdaOctetString(
          new ObjectReference(ref),
          fc,
          sAddr,
      ...
      if (val != null) {
        // TODO
        // throw new SclParseException("parsing configured value for octet string is not
        // supported
        // yet.");
      }
      return bda;
